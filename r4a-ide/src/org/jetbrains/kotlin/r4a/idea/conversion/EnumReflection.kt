...
fun AttributeInfo.getPossibleValues(project: Project): List<FqName> {
    return getPossibleValues(project, descriptor ?: return emptyList())
}
...
internal fun getPossibleValues(project: Project, descriptor: DeclarationDescriptor): List<FqName> {
    ...
    return getPossibleValuesFromAnnotations(project, valueDescriptor)
}
...
internal fun getPossibleValuesFromEnum(type: KotlinType): List<FqName> {
    return (type.constructor.declarationDescriptor as ClassDescriptor)
        ...
        .filter { it is EnumEntrySyntheticClassDescriptor || (it is LazyClassDescriptor && it.kind == ClassKind.ENUM_ENTRY) }
        .mapNotNull { it.fqNameOrNull() }
}

private fun getPossibleValuesFromAnnotations(project: Project, descriptor: DeclarationDescriptor): List<FqName> {
    ...
    if (psiElement != null && psiElement is PsiModifierListOwner) {
        fillPossibleEnumValues(project, psiElement, fqNames, hashSetOf())
    } else {
        val visited = hashSetOf<PsiClass>()
        descriptor.annotations.forEach {
            val annotationFqName = it.fqName?.asString() ?: return@forEach
            fillPossibleEnumValues(project, annotationFqName, fqNames, visited)
        }
    }
...
private fun fillPossibleEnumValues(
    ...
    val annotationClass = JavaPsiFacade.getInstance(project).findClass(annotationFqName, project.allScope()) ?: return
    ...
    fillPossibleEnumValues(project, annotated, enumFqNames, visited)
}
...
private fun fillPossibleEnumValues(
    ...
    for (annotation in annotations) {
        ...
        if (qualifiedName in MAGIC_CONSTANT_ANNOTATIONS) {
            ...
            fillPossibleEnumValues(project, qualifiedName, enumFqNames, visited)
        }
    }
}
...
private fun getMagicConstantEnumValues(annotation: PsiAnnotation): List<FqName> {
    ...
    return allowedValues
        ...
        .mapNotNull { references -> references.firstNotNullResult { it.resolve()?.getKotlinFqName() } }
}