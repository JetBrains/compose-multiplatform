...
data class ImportVariant(
    ...
    companion object {
        private val RENDERER = DescriptorRenderer.SHORT_NAMES_IN_TYPES.withOptions { parameterNamesInFunctionalTypes = false }
        ...
        fun forAttribute(descriptor: DeclarationDescriptor): ImportVariant {
            ...
            val hint = "$attrName=(${type?.let { RENDERER.renderType(it) }}) (from ${descriptor.containingDeclaration?.fqNameSafe})"

            return ImportVariant(descriptor, hint, fqName)
        }

        fun forComponent(descriptor: DeclarationDescriptor): ImportVariant {
            return ImportVariant(descriptor, descriptor.fqNameSafe.asString(), descriptor.fqNameSafe)
        }
    }
}