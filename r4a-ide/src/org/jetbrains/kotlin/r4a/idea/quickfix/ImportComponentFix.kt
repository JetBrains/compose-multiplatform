...
class ImportComponentFix(element: KtSimpleNameExpression, private val ktxElement: KtxElement) : R4aImportFix(element) {
    ...
    override fun computeSuggestions(): List<ImportVariant> {
        ...
        indicesHelper.processTopLevelCallables({ it == name }) loop@{ descriptor ->
            ...
            candidates.add(ImportVariant.forComponent(descriptor))
        }
    ...
    companion object MyFactory : KotlinSingleIntentionActionFactory() {
        override fun createAction(diagnostic: Diagnostic): IntentionAction? {
            ...
            val tagOpen = ktxElement.simpleTagName /* ?: ktxElement.qualifiedTagName */ ?: return null
            if (tagOpen.textRange.contains(element.textRange)) {
                return ImportComponentFix(element, ktxElement).apply { collectSuggestions() }
            }
            val tagClose = ktxElement.simpleClosingTagName /* ?: ktxElement.qualifiedClosingTagName */ ?: return null
