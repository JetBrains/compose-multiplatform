...
class ConvertCallToKtxQuickFix(private val expression: KtCallExpression) : KotlinQuickFixAction<KtElement>(expression) {
    companion object MyFactory : KotlinSingleIntentionActionFactory() {
        override fun createAction(diagnostic: Diagnostic): IntentionAction? {
            val target = R4AErrors.SVC_INVOCATION.cast(Errors.PLUGIN_ERROR.cast(diagnostic).a.diagnostic).psiElement
