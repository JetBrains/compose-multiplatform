...
object Weighers {
    ...
    val IsImported = object : LookupElementWeigher("r4a.importedWeigher") {
        override fun weigh(element: LookupElement, context: WeighingContext) = if (element.getUserData(IS_IMPORTED_KEY) == true) -1 else 0
    }

    abstract class AttributeInfoWeigher(key: String) : LookupElementWeigher(key) {
        override fun weigh(element: LookupElement, context: WeighingContext) = element.getUserData(ATTRIBUTE_INFO_KEY)?.let { weigh(it) } ?: 0
