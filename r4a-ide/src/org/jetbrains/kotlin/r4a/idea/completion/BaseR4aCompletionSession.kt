...
abstract class BaseR4aCompletionSession(
    ...
    protected val r4aComponentDescriptor = module.findClassAcrossModuleDependencies(ClassId.topLevel(R4aUtils.r4aFqName("Component")))
    protected val androidViewDescriptor = module.findClassAcrossModuleDependencies(ClassId.topLevel(FqName("android.view.View")))
    protected val androidViewGroupDescriptor = module.findClassAcrossModuleDependencies(ClassId.topLevel(FqName("android.view.ViewGroup")))

    protected val r4aChildrenAnnotationFqName = R4aUtils.r4aFqName("Children")
    protected val r4aComposableAnnotationFqName = R4aUtils.r4aFqName("Composable")
    ...
    protected val androidViewGroupPsiClass = psiFacade.findClass("android.view.ViewGroup", searchScope)

    protected fun DeclarationDescriptor.isVisibleDescriptor(): Boolean {
        if (this is TypeParameterDescriptor && !isTypeParameterVisible(this)) return false

        if (this is DeclarationDescriptorWithVisibility) {
            return isVisible(cursorElement, callTypeAndReceiver.receiver as? KtExpression, bindingContext, resolutionFacade)
        }
    ...
    protected fun Annotated.hasChildrenAnnotation() = annotations.hasAnnotation(r4aChildrenAnnotationFqName)
    protected fun Annotated.hasComposableAnnotation() = annotations.hasAnnotation(r4aComposableAnnotationFqName)

    protected fun DeclarationDescriptor.allowsChildren(): Boolean {
        return when (this) {
            is ClassDescriptor -> when {
                androidViewGroupDescriptor != null && isSubclassOf(androidViewGroupDescriptor) -> true
                unsubstitutedMemberScope.getContributedDescriptors().any { it.hasChildrenAnnotation() } -> true
                unsubstitutedPrimaryConstructor?.valueParameters?.any { it.hasChildrenAnnotation() } ?: false -> true
                else -> false
            }
            ...
            is VariableDescriptor -> when {
                type.isFunctionOfAnyKind() && type.arguments.any { it.type.hasChildrenAnnotation() } -> true
                else -> false
            }
            else -> false
        }
    ...
    protected fun KotlinType.functionParameterNames(): List<String> = if (isFunctionOfAnyKind()) arguments.mapNotNull {
