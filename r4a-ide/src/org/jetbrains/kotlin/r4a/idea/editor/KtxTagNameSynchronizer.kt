...
private val Document.synchronizers get() = EditorFactory.getInstance().getEditors(this).mapNotNull { it.getUserData(SYNCHRONIZER_KEY) }
 ...
 */
class KtxTagNameSynchronizer(editorFactory: EditorFactory, private val manager: FileDocumentManager) : CommandListener {
...
private class TagNameSynchronizer(val editor: Editor, val project: Project) : DocumentListener {
    ...
    fun fitsInMarker(offset: Int, oldLength: Int): Boolean {
        ...
        for (leaderAndSupport in markers) {
            ...
            fitsInMarker = fitsInMarker or (offset >= leader.startOffset && offset + oldLength <= leader.endOffset)
        }
        return fitsInMarker
    }
    ...
    private fun createTagNameMarker(caret: Caret): RangeMarker? {
        ...
        for (i in offset - 1 downTo Math.max(0, offset - 50)) {
            try {
                ...
                LOG.error("incorrect offset:$i, initial: $offset", Attachment("document.txt", sequence.toString()))
                return null
            }
    ...
    private fun findSupportForTagList(leader: RangeMarker, element: PsiElement?, document: Document): RangeMarker? {
    ...
    private fun findSupportElement(el: PsiElement?): PsiElement? {
        ...
        } else if (end != null && end.startOffset <= el.startOffset && end.endOffset >= el.endOffset) {
