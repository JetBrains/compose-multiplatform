        ...
        when (parent) {
            is KtxElement -> {
                if (element == parent.simpleTagName) {
                    ...
                    return arrayOf<PsiReference>(KtxReference(parent, element))
                }
            }
            is KtxAttribute -> {
                if (parent.key == element) {
                    // attribute key
                    return arrayOf<PsiReference>(AttributeReference(parent))
                }
            }
            is KtDotQualifiedExpression -> {
                val superparent = parent.parent
                if (superparent is KtxElement) {
                    val openFirstPart = firstNameForElement(superparent)
                    if (openFirstPart == element) {
                        ...
                        return arrayOf<PsiReference>(KtxSyntaxReferenceForImports(superparent))
                    } else {
                        val openLastPart = lastNameForElement(superparent)
                        val closingLastPart = superparent.qualifiedClosingTagName?.lastSimpleNameExpression()
                        if (closingLastPart == element || openLastPart == element) {
                            ...
                            return arrayOf<PsiReference>(KtxReference(superparent, element))
                        }
                    }
                }
            }
...
open class KtxSyntaxReference(val ktxElement: KtxElement) : KtMultiReference<KtElement>(ktxElement) {

    override fun getRangeInElement() = TextRange(0, 1)

    override fun getTargetDescriptors(context: BindingContext): Collection<DeclarationDescriptor> {
        if (!R4AFlags.USE_NEW_TYPE_RESOLUTION) return emptyList()

        val resolvedElementCall = context[R4AWritableSlices.RESOLVED_KTX_CALL, ktxElement] ?: return emptyList()
...
class AttributeReference(private var attribute: KtxAttribute) : KtSimpleReference<KtSimpleNameExpression>(attribute.key) {
    ...
    override fun getTargetDescriptors(context: BindingContext): Collection<DeclarationDescriptor> {
        return context[R4AWritableSlices.ATTRIBUTE_KEY_REFERENCE_TARGET, attribute.key]
    ...
    override val resolvesByNames: Collection<Name>
        get() = attribute.key.getReferencedNameAsName().let {
            listOf(
                it,
                Name.identifier(R4aUtils.setterMethodFromPropertyName(it.identifier))
            )
        }
}

class KtxReference(val ktxElement: KtxElement, val nameElement: KtSimpleNameExpression) : KtMultiReference<KtElement>(nameElement) {
    ...
    private fun primaryTargetOf(resolvedCall: ResolvedCall<*>?): List<DeclarationDescriptor> {
        return when (resolvedCall) {
            is VariableAsFunctionResolvedCall -> listOf(resolvedCall.variableCall.resultingDescriptor)
            null -> emptyList()
    ...
    private fun primaryTargetsOf(node: EmitOrCallNode?): List<DeclarationDescriptor> {
        return when (node) {
            ...
            is MemoizedCallNode -> primaryTargetOf(node.memoize.ctorCall) + primaryTargetsOf(node.call)
            is NonMemoizedCallNode -> primaryTargetOf(node.resolvedCall) + primaryTargetsOf(node.nextCall)
            is ErrorNode -> emptyList()
            null -> emptyList()
        }
    }

    private fun filterResultsIfInSameContainer(targets: List<DeclarationDescriptor>): List<DeclarationDescriptor> {
        ...
        for (target in targets.drop(1)) {
            ...
            if (prev is ClassConstructorDescriptor && prev.containingDeclaration == container) continue
...
private fun KtExpression.firstSimpleNameExpression(): KtSimpleNameExpression? = when (this) {
    ...
    is KtDotQualifiedExpression -> receiverExpression.firstSimpleNameExpression() ?: selectorExpression?.firstSimpleNameExpression()
    else -> null
}
...
private fun KtExpression.lastSimpleNameExpression(): KtSimpleNameExpression? = when (this) {
    is KtSimpleNameExpression -> this
    is KtDotQualifiedExpression -> selectorExpression?.lastSimpleNameExpression() ?: receiverExpression.lastSimpleNameExpression()
    else -> null
}