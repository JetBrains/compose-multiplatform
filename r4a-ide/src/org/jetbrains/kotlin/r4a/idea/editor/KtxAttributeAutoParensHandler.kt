...
class KtxAttributeAutoParensHandler : TypedHandlerDelegate() {
    ...
    private fun wrapWithParensIfAtEndOfAttribute(file: PsiFile, editor: EditorEx, project: Project): Boolean {
        ...
        val attr = errorElement.getPrevSiblingIgnoringWhitespaceAndComments(withItself = false) as? KtxAttribute ?: return false
        ...
        if (valueExpr.node.elementType == KtNodeTypes.PARENTHESIZED && valueExpr.lastChild is PsiErrorElement) {
    ...
    private fun wrapValueExpressionIfNotTagCloseForChar(c: Char, file: PsiFile, editor: EditorEx): Boolean {
        ...
        if (bracketElements.first { it.elementType == KtTokens.GT }.startOffset < offset) return false
        ...
        val attr = el.getPrevSiblingIgnoringWhitespaceAndComments(withItself = true) as? KtxAttribute ?: return false
        val valueExpr = attr.value ?: return false
