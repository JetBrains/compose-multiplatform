...
class CommonCSSValuesTest {
    ...
    fun gridAutoFlowValuesHavePortableCssText() {
        ...
        val expected = listOf("row", "column", "dense", "row dense", "column dense")
        val style = StyleScopeBuilder()
        values.forEach { style.gridAutoFlow(it) }

        assertEquals(expected, values.map { it.toString() })
        assertEquals(List(5) { "grid-auto-flow" }, style.properties.map { it.name })
        assertEquals(expected, style.properties.map { it.value.toString() })
    }
}
