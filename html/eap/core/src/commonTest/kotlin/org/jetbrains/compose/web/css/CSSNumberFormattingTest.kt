...
class CSSNumberFormattingTest {
    ...
    // Binary32 edges: invalid or inexact CSS must still match across platforms.
    @Test
    ...
    fun runtimeFloatsKeepBinary32FormattingThroughNumberApis() {
        ...
        assertEquals(listOf("opacity" to "0.33333334"), style.properties.map { it.name to it.value.toString() })
        assertEquals(listOf("alpha" to "0.33333334"), style.variables.map { it.name to it.value.toString() })
    ...
    fun runtimeFloatsKeepBinary32FormattingInTransformsAndFilters() {
        ...
        assertEquals(
            listOf(
                "transform" to "matrix(0.33333334, 0.33333334, 0.33333334, 0.33333334, 0.33333334, 0.33333334) " +
                    "scale(0.33333334) rotate3d(0.33333334, 0.33333334, 0.33333334, 90deg)",
                "filter" to "opacity(0.33333334)",
            ),
            style.properties.map { it.name to it.value.toString() },
        )
    }
