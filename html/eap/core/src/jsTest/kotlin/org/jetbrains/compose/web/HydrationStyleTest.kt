...
class HydrationStyleTest {
    @Test
    fun detachedFallbackStyleAppliesWhenAttached() = MainScope().promise {
        val root = document.createElement("div") as HTMLElement
        val target = document.createElement("div") as HTMLElement
        target.id = "detached-fallback-style-target"
        ...
        try {
            ...
            assertContains(style.textContent.orEmpty(), "color: green;")
            textColor = Color.red
            delay(100.milliseconds)
            assertContains(style.textContent.orEmpty(), "color: red;")

            root.appendChild(target)
            document.body!!.appendChild(root)
            assertEquals("rgb(255, 0, 0)", window.getComputedStyle(target).color)

            textColor = Color.blue
            delay(100.milliseconds)
            assertEquals("", style.textContent)
            assertEquals("rgb(0, 0, 255)", window.getComputedStyle(target).color)
        } finally {
            composition.dispose()
