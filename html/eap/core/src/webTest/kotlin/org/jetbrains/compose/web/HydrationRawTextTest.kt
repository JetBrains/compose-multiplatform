...
class HydrationRawTextTest {
    @Test
    fun noscriptPreservesServerFallbackAcrossRecomposition() = MainScope().promise {
        for (nested in listOf(false, true)) {
            val root = browserDocument.createElement("div") as HTMLElement
            var value by mutableStateOf("A & B")
            var fallbackCompositions = 0
            val content: @Composable () -> Unit = {
                TagElement<Element>("noscript", null) {
                    fallbackCompositions++
                    if (nested) Span { Text(value) } else Text(value)
                }
                Span { Text(value) }
            }
            root.innerHTML = composeHtmlToString(content = content)
            val noscript = root.firstChild!!
            val serverText = noscript.firstChild
            ...
            try {
                assertSame(noscript, root.firstChild)
                assertSame(serverText, noscript.firstChild)
                assertEquals(fallback, noscript.textContent)
                assertEquals(0, fallbackCompositions)

                value = "updated"
                delay(100.milliseconds)
                assertEquals(value, root.lastChild!!.textContent)
                assertSame(noscript, root.firstChild)
                assertSame(serverText, noscript.firstChild)
                assertEquals(fallback, noscript.textContent)
                assertEquals(0, fallbackCompositions)
    ...
    @Test
    fun rcdataHydratesAndUpdatesAdjacentText() = MainScope().promise {
        for (tag in listOf("title", "textarea")) {
            val root = browserDocument.createElement("div") as HTMLElement
            val prefix = "\nA & B\r"
            var suffix by mutableStateOf("</$tag>")
            ...
            try {
                assertSame(parent, root.firstChild)
                assertSame(serverText, parent.firstChild)
    ...
    @Test
    fun rcdataSplittingWaitsUntilHydrationSucceeds() {
        for (tag in listOf("title", "textarea")) {
            val root = browserDocument.createElement("div") as HTMLElement
            root.innerHTML = "<$tag>first second</$tag><span>server</span>"
    ...
    fun genericRawTextHydrationOnlyNormalizesParserInput() {
        ...
        listOf("</script>", "<!-- <script>", "first\r\nsecond\u0000").forEach { value ->
            listOf(false, true).forEach { allowed ->
                val root = browserDocument.createElement("div") as HTMLElement
                val serverScript = browserDocument.createElement("script")
    ...
    fun allowedRawTextMismatchStillRejectsNonTextChildren() {
        listOf(false, true).forEach { generic ->
            val root = browserDocument.createElement("div") as HTMLElement
            val serverScript = browserDocument.createElement("script")
            val child = browserDocument.createElement("span")
    ...
    fun mismatchFallbackExecutesInlineScriptAgain() {
        ...
        try {
            val serverScript = browserDocument.createElement("script") as HTMLScriptElement
            ...
            try {
                assertEquals(2, getWindowIntProperty(counterName))
            } finally {
                composition.dispose()
            }
        } finally {
            root.parentNode?.removeChild(root)
            clearWindowIntProperty(counterName)
        }
    }
