...
class HydrationStateTest {
    ...
    fun discoversProtocolElementsWithinTheDocumentByDefault() {
        ...
        val host = browserDocument.createElement("div")
        host.appendChild(fixture.root)
        host.appendChild(fixture.state)
        assertNotNull(browserDocument.body).appendChild(host)
    ...
    fun anUnrelatedNodeBetweenProtocolElementsIsAllowed() {
        ...
        fixture.host.insertBefore(browserDocument.createComment("injected"), fixture.state)
    ...
    fun malformedStateElementFailsWithoutChangingTheRoot() {
        val mutations: List<(Fixture) -> Unit> = listOf(
            ...
            { fixture ->
                val replacement = browserDocument.createElement("div")
    ...
    private fun decodeTestState(json: String): TestState {
        val parsed = decodeHydrationTestState(json)
        return TestState(
            label = parsed.label,
            count = parsed.count,
        )
