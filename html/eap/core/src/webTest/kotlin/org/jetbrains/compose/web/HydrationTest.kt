...
class HydrationTest {
    ...
    fun hydrationChecksAndCanRepairAttributeNamespaces() {
        ...
        listOf(false, true).forEach { allowMismatch ->
            val root = browserDocument.createElement("div") as HTMLElement
            val svg = browserDocument.createElementNS(svgNamespace, "svg")
    ...
    fun hydrationDoesNotApplyHtmlBooleanRulesToSvgAttributes() {
        val root = browserDocument.createElement("div") as HTMLElement
        val serverElement = browserDocument.createElementNS(svgNamespace, "sparkline")
    ...
    fun failedInitializationStopsItsRecomposer() = MainScope().promise {
        for (mode in listOf("hydration", "fallback", "render")) {
            ...
            val failure = IllegalStateException("ref failed")
            val content: @Composable () -> Unit = {
                Div({ ref { throw failure } })
            }
            ...
            delay(100.milliseconds)
            assertTrue((Recomposer.runningRecomposers.value - before).isEmpty(), mode)
        }
    }

    @Test
    fun tagNamesUseAsciiCaseComparison() {
        val root = browserDocument.createElement("div") as HTMLElement
    ...
    @Test
    fun disposingCompositionStopsItsRecomposer() = MainScope().promise {
        for (mode in listOf("hydration", "fallback", "render")) {
            val root = browserDocument.createElement("div") as HTMLElement
            val before = Recomposer.runningRecomposers.value
            val composition = if (mode == "render") {
            ...
            try {
                // Fallback may also have a failed attempt that is still shutting down.
                assertTrue(owned.isNotEmpty(), mode)
            } finally {
                composition.dispose()
            }
            delay(100.milliseconds)
            assertTrue(owned.none { it in Recomposer.runningRecomposers.value }, mode)
        }
    }
    ...
    @Test
    fun leadingEmptyTextPreservesComposedRootWhitespace() {
        for (withFollowingText in listOf(false, true)) {
            val root = browserDocument.createElement("div") as HTMLElement
            val content: @Composable () -> Unit = {
                Text("")
                Text(" ")
                if (withFollowingText) Text("end")
            }
            root.innerHTML = composeHtmlToString(content = content)
            ...
            try {
                assertEquals(if (withFollowingText) 3 else 2, root.childNodes.length)
                assertEquals("", root.firstChild!!.textContent)
                assertSame(whitespace, root.childNodes.item(1))
                assertEquals(if (withFollowingText) " end" else " ", root.textContent)
            } finally {
                composition.dispose()
            }
        }
    ...
    @Test
    fun leadingEmptyTextStillAllowsRootFormattingWhitespace() {
        val root = browserDocument.createElement("div") as HTMLElement
        root.innerHTML = "\n <span>end</span>\n"
        ...
        try {
            assertEquals(2, root.childNodes.length)
            assertEquals("", root.firstChild!!.textContent)
            assertSame(span, root.lastChild)
        } finally {
            composition.dispose()
        }
    }
    ...
    @Test
    fun abortedHydrationCanBeClearedWithoutMutatingServerDom() {
        val root = browserDocument.createElement("div") as HTMLElement
