...
class HydrationAttributesTest {
    @Test
    fun customAutoCompleteValueHydrates() {
        ...
        try {
            ...
            assertEquals("search", input.getAttribute("autocomplete"))
        } finally {
            composition.dispose()
        }
    }
    ...
    @Test
    fun customElementBooleanNamedAttributesPreserveValuesDuringHydration() {
        val root = document.createElement("div") as HTMLElement
        root.innerHTML = composeHtmlToString {
        ...
        try {
            assertSame(widget, root.firstChild)
            assertEquals("details", widget.getAttribute("open"))
            assertEquals("false", widget.getAttribute("checked"))
        } finally {
            composition.dispose()
        }
    }
    ...
    @Test
    fun customElementBooleanNamedAttributeMismatchesAreNotIgnored() {
        listOf("open", "checked").forEach { name ->
            val root = document.createElement("div") as HTMLElement
            ...
            assertSame(widget, root.firstChild)
            assertEquals("", widget.getAttribute(name))
        }
    }
    ...
    fun matchingAttributesClassesAndStylesAreNotRewritten() {
        ...
        observer.observe(
            ...
            MutationObserverInit(
                attributes = true,
                attributeFilter = listOf("class", "style", "data-kind")
                    .map(String::toJsString)
                    .toJsArray(),
            ),
        )
