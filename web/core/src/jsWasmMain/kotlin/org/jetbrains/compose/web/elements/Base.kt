...
import org.jetbrains.compose.web.internal.runtime.getW3cListener
...
private class DomElementWrapper(override val node: Element): DomNodeWrapper(node) {
    ...
    fun updateEventListeners(list: List<NamedEventListener>) {
        currentListeners.forEach {
            node.removeEventListener(it.name, it.getW3cListener())
        }

        currentListeners = list

        currentListeners.forEach {
            node.addEventListener(it.name, it.getW3cListener())
        }
    }
