...
fun PixelMap.assertPixelColor(
    ...
    assertEquals(errorString, expected.red, color.red)
    assertEquals(errorString, expected.green, color.green)
    assertEquals(errorString, expected.blue, color.blue)
    assertEquals(errorString, expected.alpha, color.alpha)
}
...
fun Path.contains(offset: Offset): Boolean {
    return asSkiaPath().contains(offset.x, offset.y)
}
