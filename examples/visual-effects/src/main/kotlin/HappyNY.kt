...
data class SnowFlake(var x: Dp, var y: Dp, val scale: Float, var v: Double, var alpha: Float, var angle : Float, var rotate : Int, var phase : Double)
...
class DoubleRocket(val particle: Particle) {
    ...
    fun explode(time: Long) {
        ...
        rockets = Array<Rocket>(7) {
            ...
            Rocket(Particle(particle.x, particle.y,v*sin(angle)+particle.vx, v*cos(angle)+particle.vy-0.5f, colors[it % colors.size]), colors[it % colors.size], time)
        }
...
class Particle(var x: Double, var y: Double, var vx: Double, var vy : Double, val color : Color, val type: Int = 0) {
    ...
    fun draw() {
        ...
        for (i in 1..5) {
            Box(Modifier.size(4.dp).offset((x-vx/2*i).dp, (y-vy/2*i).dp).alpha(alphaFactor*(1-0.18f*i)).clip(CircleShape).background(color))
        }
    }
}
...
fun prepareStarsAndSnowFlakes(stars : SnapshotStateList<Star>, snowFlakes: SnapshotStateList<SnowFlake>) {
    for (i in 0..snowCount) {
        snowFlakes.add(SnowFlake((50 + (width-50)*random()).dp, (height*random()).dp, 0.1f+0.2f*random().toFloat(), 1.5+3*random(), (0.4f+0.4*random()).toFloat(), 60*random().toFloat(), Random.nextInt(1, 5) - 3, random()*2*PI))
    }
    val colors = arrayOf(Color.Red, Color.Yellow, Color.Green, Color.Yellow, Color.Cyan, Color.Magenta, Color.White)
    for (i in 0..starCount) {
        stars.add(Star( (width*random()).dp, (height*random()).dp, colors[Random.nextInt(colors.size)], (3 + 5*random()).dp))
    }
}
...
fun NYContent() {
    ...
    Surface(
        modifier = Modifier.fillMaxSize().padding(5.dp).shadow(3.dp, RoundedCornerShape(20.dp)).pointerMoveFilter(onMove = {false;},
        ...
        with (LocalDensity.current) {
            Box(Modifier.fillMaxSize()) {
                ...
                Text("202", Modifier.scale(10f).align(Alignment.Center).offset(-2.dp, 15.dp).alpha(if (flickering2) 0.8f else 1.0f), color = Color.White)

                val alpha = if (flickering2) flickeringAlpha(time) else 1.0f
                Text("2", Modifier.scale(10f).align(Alignment.Center).offset(14.dp, 15.dp).alpha(alpha), color = Color.White)
                ...
                if (started ) { //delay to be able start recording
                    ...
                    HNYString.forEach {
                        ...
                        Text(it.toString(), color = color, modifier = Modifier.scale(5f).align(Alignment.Center).offset(0.dp, 85.dp).rotate((angle + 5.0f*i)).offset(0.dp, -90.dp).alpha(alpha))
                        i++
                    }
                ...
                Text("Powered by Compose Multiplatform", modifier = Modifier.align(Alignment.BottomEnd).offset(-20.dp, 0.dp), color = Color.White)
            }
        }
    }
}
...
fun blend(color1 : Color, color2 : Color, fraction : Float) : Color {
    ...
    return Color(color2.red*fraction + color1.red*(1-fraction), color2.green*fraction + color1.green*(1-fraction), color2.blue*fraction + color1.blue*(1-fraction))
}
...
fun snowFlakeInt(level: Int, angle: Float, shiftX: Dp, shiftY: Dp, alpha: Float) {
    if (level > 3) return
    Box(Modifier.offset(shiftX, shiftY).rotate(angle).width(100.dp).height(10.dp).scale(0.6f).alpha(1f).background(Color.White.copy(alpha = alpha))) {
