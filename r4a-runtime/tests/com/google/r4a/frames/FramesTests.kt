...
class FrameTest: TestCase() {
    ...
    fun testModelList_Isolated() {
        val addresses = frame {
            modelListOf(*(Array(100) { Address(OLD_STREET, OLD_CITY) }))
        }
        ...
        val frame1 = suspended {
            ...
            addresses.add(Address(NEW_STREET, NEW_CITY))
    ...
    fun testModelList_ReadDoesNotModify() {
        val count = 10
        val addresses = frame {
            modelListOf(*(Array(count) { Address(OLD_STREET, OLD_CITY) }))
        }
    ...
    fun testModelList_MutateThrows() {
        ...
        frame {
            ...
            iterator.next()
            expectError {
                iterator.set(Address(NEW_STREET, NEW_CITY))
            }
            assertFalse(wasModified(addresses))
        }
        ...
        frame {
            ...
            iterator.next()
            expectError {
                iterator.add(Address(NEW_STREET, NEW_CITY))
            }
            assertFalse(wasModified(addresses))
        }
    }
    ...
    fun testModelList_MutatingModifies() {
        ...
        validate { addresses.add(Address(NEW_STREET, OLD_CITY)) }
        validate { addresses.add(5, Address(NEW_STREET, OLD_CITY)) }

        // Expect addAll to modify
        validate { addresses.addAll(listOf(Address(NEW_STREET, NEW_CITY), Address(NEW_STREET, NEW_CITY))) }
        validate { addresses.addAll(5, listOf(Address(NEW_STREET, NEW_CITY), Address(NEW_STREET, NEW_CITY))) }
        ...
        validate { addresses[5] = Address(NEW_STREET, NEW_CITY) }
    ...
    fun testModelMap_Isolated() {
        val map = frame { modelMapOf(1 to "a", 2 to "b", 3 to "c", 4 to "d") }
    ...
    fun testModelMap_MutateThrows() {
        ...
        validate { map.entries.addAll(listOf(map.entries.first(), map.entries.drop(1).first())) }
        validate { map.entries.clear() }
        validate { map.entries.remove(map.entries.first()) }
        validate { map.entries.removeAll(listOf(map.entries.first(), map.entries.drop(1).first())) }
        validate { map.entries.retainAll(listOf(map.entries.first(), map.entries.drop(1).first())) }
        validate {
        ...
        validate { map.values.removeAll(listOf(map.values.first(), map.values.drop(1).first())) }
        validate { map.values.retainAll(listOf(map.values.first(), map.values.drop(1).first())) }
        validate {
