...
class Effect<T> internal constructor(
     ...
     */
    @Hide
    fun constructKey(key: Int) = if (dataKey === absentKey) key else joinKey(key, dataKey)
 ...
 */
fun <T> effectOf(block: Effect<T>.() -> T): Effect<T> = Effect(block)
...
@PublishedApi
internal class CommitScopeImpl(internal val onCommit: CommitScope.() -> Unit) : CommitScope, CompositionLifecycleObserver {
    private var disposeCallback = emptyDispose

    override fun onDispose(callback: () -> Unit) {
        assert(disposeCallback === emptyDispose) { "onDispose(...) should only be called once" }
 ...
 */
@CheckResult(suggest = "+")
fun <T, V1> key(v1: V1, block: Effect<T>.() -> T) = Effect(block, v1)
 ...
 */
@CheckResult(suggest = "+")
fun <T, V1, V2> key(v1: V1, v2: V2, block: Effect<T>.() -> T) = Effect(block, joinKey(v1, v2))
 ...
 */
@CheckResult(suggest = "+")
fun <T> key(vararg inputs: Any?, block: Effect<T>.() -> T) = Effect(block, inputs.reduce { acc, item -> joinKey(acc, item) })
