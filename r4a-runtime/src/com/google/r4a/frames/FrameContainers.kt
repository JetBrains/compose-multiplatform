...
class ModelList<T> : MutableList<T>, Framed {
    private var myFirst: Record = ArrayContainer<T>()
    ...
    @Suppress("UNCHECKED_CAST") private val readable: ArrayContainer<T> get() = _readable(myFirst, this) as ArrayContainer<T>
    @Suppress("UNCHECKED_CAST") private val writable: ArrayContainer<T> get() = _writable(myFirst, this) as ArrayContainer<T>
    ...
    override fun addAll(index: Int, elements: Collection<T>): Boolean = writable.list.addAll(index, elements)
    ...
    override fun iterator(): MutableIterator<T> = immutableIterator(readable.list.iterator())
    override fun lastIndexOf(element: T): Int = readable.list.lastIndexOf(element)
    override fun listIterator(): MutableListIterator<T> = immutableListIterator(readable.list.listIterator())
    override fun listIterator(index: Int): MutableListIterator<T> = immutableListIterator(readable.list.listIterator(index))
    ...
    override fun subList(fromIndex: Int, toIndex: Int): MutableList<T> = writable.list.subList(fromIndex, toIndex)
...
class ModelMap<K, V> : MutableMap<K, V>, Framed {
    private var myFirst: Record = MapContainer<K, V>()
    override val firstFrameRecord: Record get() = myFirst
    ...
    private val readable: MapContainer<K, V>
        get() = _readable(myFirst, this) as MapContainer<K, V>
    @Suppress("UNCHECKED_CAST")
    private val writable: MapContainer<K, V>
        get() = _writable(myFirst, this) as MapContainer<K, V>
...
private fun error(): Nothing = error("Model sub-collection, iterators, lists and sets are immutable, use asMutable() first")
...
private fun <T> immutableSet(set: MutableSet<T>): MutableSet<T> = object : MutableSet<T> by set {
    ...
    override fun iterator(): MutableIterator<T> = immutableIterator(set.iterator())
...
private fun <T> immutableIterator(iterator: MutableIterator<T>) = object : MutableIterator<T> by iterator {
    override fun remove() = error()
}

private fun <T> immutableListIterator(iterator: MutableListIterator<T>) = object : MutableListIterator<T> by iterator {
    override fun add(element: T) = error()
    override fun remove() = error()
    override fun set(element: T) = error()
}

private fun <T> immutableCollection(collection: MutableCollection<T>) = object : MutableCollection<T> by collection {
    override fun add(element: T): Boolean = error()
    override fun addAll(elements: Collection<T>): Boolean = error()
    override fun clear() = error()
    override fun iterator(): MutableIterator<T> = immutableIterator(collection.iterator())
    override fun remove(element: T): Boolean = error()
    override fun removeAll(elements: Collection<T>): Boolean = error()
    override fun retainAll(elements: Collection<T>): Boolean = error()
}
