...
open class SlotEditor internal constructor(val table: SlotTable) {
     ...
     */
    fun get(index: Int) = effectiveIndex(index).let { if (it < slots.size) slots[it] else SlotTable.EMPTY }
    ...
    internal fun recordEndGroup(writing: Boolean, inserting: Boolean, uncertain: Boolean): Int {
        ...
        assert(slots[effectiveStartLocation] === SlotTable.EMPTY || slots[effectiveStartLocation] is GroupStart) { "Invalid state. Start location stack doesn't refer to a start location" }
        ...
        if (writing) {
            ...
            assert(start.slots == len && (nodeCount == start.nodes || uncertain)) { "Invalid endGroup call, expected ${start.slots} slots and ${start.nodes} nodes but received, $len slots and $nodeCount nodes" }
            count = start.nodes
        }
...
class SlotReader internal constructor(table: SlotTable) : SlotEditor(table) {
    ...
    /**
     * Get the value of the next slot. During empty mode this value is always EMPTY which is the value a
     * newly inserted slot.
     */
    fun next(): Any? {
        if (emptyCount > 0) {
    ...
    override fun toString(): String {
        return "${javaClass.simpleName}(current=$current, size=${slots.size - table.gapLen}, gap=${if (table.gapLen > 0) "$table.gapStart-${table.gapStart + table.gapLen - 1}" else "none"}${if (inEmpty) ", in empty" else ""})"
    }
}
...
class SlotWriter internal constructor(table: SlotTable) : SlotEditor(table) {
     ...
     * End the current group. Must be called after the corresponding startGroup().
     */
    fun endGroup(): Int = recordEndGroup(writing = true, inserting = insertCount > 0, uncertain = false)
    ...
    private fun moveGapTo(index: Int) {
        if (table.gapLen > 0 && table.gapStart != index) {
            ...
            if (index < table.gapStart) {
                ...
                System.arraycopy(slots, table.gapStart + table.gapLen, slots, table.gapStart, len)
            }
    ...
    override fun toString(): String {
        ...
        return "${javaClass.simpleName}(current=$current, size=${slots.size - table.gapLen}, gap=${if (table.gapLen > 0) "$table.gapStart-${table.gapStart + table.gapLen - 1}" else "none"}${if (insertCount > 0) ", inserting" else ""})"
    }
...
class SlotTable(internal var slots: Array<Any?> = arrayOf()) {
    ...
    fun <T> read(block: (reader: SlotReader) -> T): T = openReader().let { reader -> block(reader).also { reader.close() } }
    fun <T> write(block: (writer: SlotWriter) -> T): T = openWriter().let { writer -> block(writer).also { writer.close() } }
