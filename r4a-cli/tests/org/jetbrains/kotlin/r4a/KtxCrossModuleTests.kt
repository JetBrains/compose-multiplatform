...
class KtxCrossModuleTests : AbstractCodeGenTest() {
    ...
    fun compose(mainClassName: String, modules: Map<String, Map<String, String>>, dumpClasses: Boolean = false): MultiCompositionTest {
        ...
        val appClasses = classLoader(modules["Main"] ?: error("No Main module specified"), dumpClasses).allGeneratedFiles

        // Load the files looking for mainClassName
        val allClasses = (libraryClasses + appClasses).filter { it.relativePath.endsWith(".class") }
    ...
    private var classesDirectory = KotlinTestUtils.tmpDir("kotlin-${testLocalUnique++}-classes")
...
fun OutputFile.writeToDir(directory: File) = FileUtil.writeToFile(File(directory, relativePath), asByteArray())
fun Collection<OutputFile>.writeToDir(directory: File) = forEach { it.writeToDir(directory) }

private fun composeMulti(composable: () -> Unit, advance: () -> Unit) = MultiCompositionTest(composable, advance)
