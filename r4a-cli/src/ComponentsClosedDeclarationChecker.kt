...
class ComponentsClosedDeclarationChecker : DeclarationChecker, StorageComponentContainerContributor {
    ...
    override fun check(declaration: KtDeclaration, descriptor: DeclarationDescriptor, context: DeclarationCheckerContext) {
        ...
        if (!ComponentMetadata.Companion.isR4AComponent(descriptor)) return
        ...
        if (declaration.hasModifier(KtTokens.OPEN_KEYWORD) || declaration.hasModifier(KtTokens.ABSTRACT_KEYWORD)) {
            ...
            context.trace.reportFromPlugin(R4AErrors.OPEN_COMPONENT.on(element), R4ADefaultErrorMessages)
        }
    }
}
