...
sealed class EmitOrCallNode {
    ...
    private fun collectPossibleAttributes(collector: MutableMap<String, MutableList<AttributeMeta>>) {
        when (this) {
            is ErrorNode -> Unit
            is MemoizedCallNode -> {
                collectPossibleAttributes(memoize.ctorCall?.resultingDescriptor as? FunctionDescriptor, collector)
                call.collectPossibleAttributes(collector)
            }
            is NonMemoizedCallNode -> {
                collectPossibleAttributes(resolvedCall.resultingDescriptor as? FunctionDescriptor, collector)
    ...
    private fun collectPossibleAttributes(callDescriptor: FunctionDescriptor?, collector: MutableMap<String, MutableList<AttributeMeta>>) {
        callDescriptor?.let {
